...
class UploadQueueManager {
  ...
  private isProcessing = false; // Prevent concurrent queue processing
  private updateCallback:
    | ((id: string, updates: Partial<FileMetadata>) => void)
    | null = null;
  ...
  private async processQueue() {
    // Prevent concurrent queue processing to avoid infinite loops
    ...
    try {
      ...
      while (
        ...
        if (file && (file.status === 'pending' || file.status === 'paused')) {
          // Don't await - let it run in parallel, catch errors separately
          this.startUpload(file).catch(error => {
  ...
  async resumeUpload(file: FileMetadata) {
    ...
    if (file.uploadId) {
      try {
        const {uploadedChunks, totalChunks} = await uploadService.resumeUpload(
          file,
        );
